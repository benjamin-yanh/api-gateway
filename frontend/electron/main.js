...
function analyzeError(errorLogs) {
  ...
  // 检测端口占用错误
  if (
    allLogs.includes('failed to start HTTP server') ||
    allLogs.includes('bind: address already in use') ||
    (allLogs.includes('listen tcp') &&
      allLogs.includes('bind: address already in use'))
  ) {
    return {
      type: '端口被占用',
  ...
  // 检测数据库错误
  if (
    allLogs.includes('database is locked') ||
  ...
  // 检测权限错误
  if (
    allLogs.includes('permission denied') ||
  ...
  // 检测网络错误
  if (
    allLogs.includes('network is unreachable') ||
  ...
  // 检测配置文件错误
  if (
    allLogs.includes('invalid configuration') ||
  ...
  // 检测内存不足
  if (
    allLogs.includes('out of memory') ||
  ...
  // 检测文件不存在错误
  if (
    allLogs.includes('no such file or directory') ||
  ...
  return new Promise((resolve, reject) => {
    ...
    const tryConnect = () => {
      ...
      if (currentAttempt % 5 === 1 && currentAttempt > 1) {
        console.log(
          `Attempting to connect to port ${port}... (attempt ${currentAttempt}/${maxRetries})`
        )
      }
      ...
      const req = http.get(
        ...
        (res) => {
          ...
          req.destroy()
          console.log(
            `✓ Successfully connected to port ${port} (status: ${res.statusCode})`
          )
          resolve()
        }
      )
      ...
      req.on('error', (err) => {
        if (currentAttempt >= maxRetries) {
          reject(
            new Error(
              `Failed to connect to port ${port} after ${maxRetries} attempts: ${err.message}`
            )
          )
        } else {
      ...
      req.on('timeout', () => {
        ...
        if (currentAttempt >= maxRetries) {
          reject(
            new Error(
              `Connection timeout on port ${port} after ${maxRetries} attempts`
            )
          )
        } else {
          setTimeout(tryConnect, retryDelay)
...
function startServer() {
  return new Promise((resolve, reject) => {
    ...
    if (isDev) {
      ...
      console.log('  1. Go backend: cd backend && go run . (port 3000)')
      ...
      checkServerAvailability(DEV_FRONTEND_PORT)
        ...
        .catch((err) => {
          console.error(
            `✗ Cannot connect to frontend dev server on port ${DEV_FRONTEND_PORT}`
          )
