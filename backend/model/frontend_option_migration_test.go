...
func TestMigrateRetiredFrontendOptionsUpdatesOnlyLegacyDefaultBrand(t *testing.T) {
	for _, legacyDefault := range legacyDefaultSystemNames {
		t.Run(legacyDefault, func(t *testing.T) {
			db := useFrontendOptionMigrationDB(t)
			require.NoError(t, db.Create(&Option{Key: "SystemName", Value: legacyDefault}).Error)
...
func TestRetiredLogoOptionIsPersistedButNotPublished(t *testing.T) {
	db := useFrontendOptionMigrationDB(t)
	previousMap := common.OptionMap
	t.Cleanup(func() { common.OptionMap = previousMap })
	common.OptionMap = map[string]string{}

	require.NoError(t, UpdateOption(retiredLogoOptionKey, "https://example.com/legacy-logo.png"))
	assert.Equal(t, "https://example.com/legacy-logo.png", requireOptionValue(t, db, retiredLogoOptionKey))
	_, published := common.OptionMap[retiredLogoOptionKey]
	assert.False(t, published)
}
