...
func GetTopUpInfo(c *gin.Context) {
	...
	data := gin.H{
		...
		// Redemption cards are an internal balance-credit mechanism and do not
		// depend on third-party payment gateway compliance confirmation.
...
func RequestEpay(c *gin.Context) {
	...
	storedPayMoney, err := decimal.NewFromString(payMoneyText)
	if err != nil {
	...
	topUp := &model.TopUp{
		UserId:          id,
		Amount:          amount,
		Money:           storedPayMoney.InexactFloat64(),
		TradeNo:         tradeNo,
...
func EpayNotify(c *gin.Context) {
	...
	if verifyInfo.TradeStatus == epay.StatusTradeSuccess {
		topUp, quotaToAdd, settleErr := model.SettleEpayTopUp(verifyInfo.ServiceTradeNo, verifyInfo.TradeNo, verifyInfo.Type, verifyInfo.Money)
		if settleErr != nil {
			logger.LogWarn(c.Request.Context(), fmt.Sprintf("易支付 充值结算被拒绝 trade_no=%s upstream_trade_no=%s callback_type=%s money=%s client_ip=%s error=%q", verifyInfo.ServiceTradeNo, verifyInfo.TradeNo, verifyInfo.Type, verifyInfo.Money, c.ClientIP(), settleErr.Error()))
			_, _ = c.Writer.Write([]byte("fail"))
			return
		}
		if quotaToAdd > 0 {
