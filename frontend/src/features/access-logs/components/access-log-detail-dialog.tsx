...
import { formatAccessLogJSON } from '../lib/format'
...
function JSONBlock(props: { label: string; value: string }) {
  return (
    <section className='grid gap-2'>
      <h3 className='text-sm font-medium'>{props.label}</h3>
      <pre className='bg-muted max-h-72 overflow-auto rounded-lg border p-3 font-mono text-xs break-all whitespace-pre-wrap'>
        {formatAccessLogJSON(props.value)}
      </pre>
    </section>
  )
}
...
function AccessLogDetailContent(props: { detail: AccessLogDetail }) {
  ...
  return (
    <div className='grid gap-5'>
      ...
      <JSONBlock label={t('Request Headers')} value={detail.headers} />
      {detail.body ? (
        <JSONBlock label={t('JSON Request Body')} value={detail.body} />
      ) : (
        <section className='grid gap-2'>
    ...
    <Dialog open={props.open} onOpenChange={props.onOpenChange}>
      <DialogContent className='max-h-[90vh] sm:max-w-3xl'>
        <DialogHeader>
          ...
          <DialogDescription>
            {t('Sensitive credentials are redacted before storage.')}
          </DialogDescription>
        </DialogHeader>
